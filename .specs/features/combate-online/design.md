...
**Atualização pós-investigação (T11, 2026-08-26):** a alternativa cogitada acima — PIE multiplayer automatizado — não pôde nem começar a ser tentada. `Build.sh BattleSquareServer` falhou com `Server targets are not currently supported from this engine distribution`: a engine instalada é a build binária do Epic Games Launcher, e builds de `TargetType.Server` só existem em engines compiladas da fonte (ver STATE.md, AD-020/B-004). As três linhas ❌ permanecem ❌ — não por limitação de ferramenta headless (`-nullrhi`, ausência de GUI), que era a hipótese original, mas por uma restrição de distribuição do engine que só se resolve trocando a instalação, decisão de infraestrutura fora do escopo desta feature. `ListenServer` (mesmo `TargetType.Game`) não tem essa restrição e continua disponível para uma investigação futura, mas ele não seria o modo de produção pretendido (DP-online-01 já registra por quê).
