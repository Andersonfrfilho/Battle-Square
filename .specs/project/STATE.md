...
**Trabalho atual:** **M7 (Contas e Moderação) — CONCLUÍDO. Roadmap percorrido até onde esta máquina permite: M1–M5 e M7 fechados; M6 é o único incompleto, por bloqueio de infraestrutura.** As duas features de M7 vivem em `apps/api-battle-pets` e **não tocam a Unreal** (o cliente C++ ficou fora de escopo por decisão das duas specs — a integração Unreal↔conta não foi decidida, e sincronizar coleção exige resolver conflito entre save local e remoto). **Conta de Jogador:** paga a dívida que `colecao-e-captura` ("a coleção é local porque M7 não existe ainda") e AD-017 já cobravam. Tudo que é decisão é puro e testável sem banco, com segredo e tempo **injetados** — token HMAC compacto (escolhido em vez de JWT porque um formato de um algoritmo só não tem campo `alg`, logo não tem a família de ataques de confusão de algoritmo), política de senha devolvendo todos os motivos de uma vez, normalização/máscara de e-mail, e limitador de tentativas por conta. Duas sutilezas de segurança com comentário no código para ninguém "otimizar" fora: o caminho de e-mail inexistente gasta o mesmo tempo do de senha errada (senão o TEMPO vira o oráculo de enumeração que a mensagem idêntica escondeu), e o refresh token é guardado como hash. Renovação rotativa com reuso recusado. **Moderação e Banimento:** o Nível 2 que AD-017 prometeu. `resolveBanState` é puro (levantado/expirado/permanente, e "mais restritivo vence" — sem essa regra, quem decidiria é a ordem que o banco devolveu); levantar marca `lifted_at` e **nunca** faz `DELETE`; registrar evidência **não** bane (automatizar puniria falso positivo em silêncio); e o banimento é checado na verificação do token, não só na emissão — custa uma consulta por requisição autenticada, e o design diz por que se paga. **Este backend não tinha UM teste; agora tem 25**, todos sem banco. Duas migrations aditivas (`0001` conta, `0002` moderação), nenhum `ALTER`/`DROP` em tabela existente. Bateria final: **`bun test` 25/0, `tsc --noEmit` limpo, Unreal intocada e ainda 118/0 + 52/0, três sondas `exit 0`.** Os 16 itens dos dois roteiros manuais seguem **não verificados** — exigem Postgres de pé. **M6 continua incompleto por B-006/B-006b/B-007 (nenhuma plataforma móvel compila nesta máquina) e B-008 (console exige licença e devkit)** — nenhum resolvível por código.
