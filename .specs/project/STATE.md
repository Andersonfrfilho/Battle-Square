...
**Trabalho atual:** **DP-ia-04 (esconderijos) e DP-02 (coabitação), as duas decididas pelo usuário e implementadas no núcleo.** Camuflar, voar e submergir entraram como REGRA, não visual, com trocas DISTINTAS entre si — o risco do pedido era virarem três nomes para "esquivar melhor". Camuflar barra físico E magia (é o que a separa de Esquivar) e custa a ação seguinte; voar barra físico e o dano de casa, mas a magia acerta 50% mais forte; submergir barra tudo e custa mover E atacar na seguinte. A cobrança do slot seguinte viaja dentro do `PostureFlags`, e não num campo novo, porque campo novo entraria no hash do estado e invalidaria os snapshots de determinismo de cenários que nem usam estas ações — os snapshots seguiram válidos. **DP-02 foi INVERTIDA**: dois pets não ocupam mais a mesma casa; os dois indo para o mesmo ponto ficam onde estavam e trombam, com golpe mútuo resolvido pelo MESMO caminho de dano do ataque, que é o que faz as posturas já registradas valerem sem reescrever nada. Trocar de casas continua permitido, e é por isso que a decisão passou a ser por POSIÇÃO FINAL: checar a casa de destino contra o estado vivo bloquearia a troca e deixaria o resultado dependente da ordem de processamento. A busca por oponente coabitando saiu de `ResolveTarget` junto. **Na tela**, como a regra do projeto exige: o evento de postura passou a carregar QUAL postura (sem isso a tela diria "assumiu postura" para as três), cada uma tem frase própria, o encontro tem a sua, e o OLHAR conta a história — voando o outro olha para cima, submerso para baixo, camuflado o olhar FICA onde estava, porque girar para o lugar certo entregaria o que a camuflagem esconde. O jogador alcança as três por C/V/B no teclado; os botões ainda não existem no `WBP` porque o plugin de edição não estava disponível na sessão (`Button_Camuflar`/`Button_Voar`/`Button_Submergir`, já esperados por `BindWidgetOptional`). Bateria: **212/212** (149 BattleSquare + 63 BattleSim), quatro sondas limpas.
