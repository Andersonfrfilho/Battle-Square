...
**Trabalho atual:** **Interface de Batalha — completa (T1–T6). O jogo ficou JOGÁVEL pela primeira vez.** O combate existia e era testado desde M1, mas ninguém conseguia jogá-lo: DP-08 adiou o layout para uma autoria visual que nunca veio. Agora há `WBP_BattleActionSelector` (6 tipos, grade 3x3 de direção, desfazer/confirmar), `ABattleScreenGameMode` que abre uma batalha DIRETO — sem mundo aberto, prioridade declarada pelo usuário — e o nível `BattleScreen`. DP-ui-01 governa: a tela não decide nada, todo botão encaminha ao `UBattleActionQueueComponent`. **Sete defeitos sérios apareceram JOGANDO, e nenhum era pegável headless:** pets invisíveis (`APetView` não tinha componente visual nem RootComponent), esferas meio enterradas, batalha de UM turno só (a fila nunca reabria — e um teste AFIRMAVA isso como contrato, L-034), três ações instantâneas (`PlayTrace` sem tempo), "Baixo" andando para a direita (eixos da grade contra os da câmera), a tela usando a casa inicial para sempre, e toda partida começando na mesma sequência aleatória (`Random.State` nascia 0 e ninguém semeava — oponente previsível anula o commit às cegas). Todos corrigidos e com teste. **Ferramental de depuração, a pedido do usuário:** `FBattleDebugScreen` + `ABattleDebugHUD` desenham um painel fixo e copiável (F9 copia e grava `Saved/BattleDebug.txt`, F10 limpa), a grade 3x3 aparece no mundo com coordenadas e ocupantes, e há indicador de fase da reprodução. `CLAUDE.md` (novo, na raiz) registra a regra de que todo comportamento novo apareça na tela, e a ordem de eficácia para depurar — **teste que reproduz vem antes de conserto** (L-035). Bateria: **191/191** (139 BattleSquare + 52 BattleSim), três sondas limpas. `BattleSim` sem uma linha tocada. Próximo: legibilidade do combate (vida, dano, quem venceu), ou voltar ao mundo aberto.
