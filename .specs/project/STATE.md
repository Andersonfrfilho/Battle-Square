...
**Trabalho atual:** **Sala e Pareamento Simples (M2)** — `spec.md`/`design.md`/`tasks.md` escritos. **Fases 1–2 (T1–T6) implementadas e verificadas**: `UBattleRoomRegistry` — código de sala único entre salas vivas (exclui `0`/`O`/`1`/`I`), `CreateRoom`/`JoinRoom` distinguindo `NotFound`/`Full`, desconexão/reconexão por segredo (`FGuid`), timeout de abandono (🧠 T5, corrida testada: reconexão antes cancela, dois lados juntos não declara vencedor, sem disparo duplo), sala vazia destruída com código reciclável. Bateria: **56/56 testes headless, Fail: 0**. Próximo: Fase 3 (T7–T9, `ABattleSquareGameMode` + `ABattleSquarePlayerController`).
