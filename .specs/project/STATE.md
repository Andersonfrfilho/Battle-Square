...
**Trabalho atual:** **Combate Online (M2)** — `spec.md`, `design.md` e `tasks.md` escritos. **Fase 1 (T1–T3) implementada e verificada**: `FNetTurnCommit` (3 campos nomeados, `Public/Net/BattleNetTypes.h`) + `ValidateNetTurnCommit` (rejeita enum fabricado fora de range) + conversão `ToTurnCommit`/`ToNetTurnCommit` (ida e volta preserva as 3 ações, testado alimentando o resolvedor real). `BattleNetConstants.h` com `CommitTimeoutSeconds=45`/`AbandonTimeoutSeconds=120`. Bateria completa: **34/34 testes headless, Fail: 0** (30 anteriores + 4 novos `BattleSquare.Net.*`). Sessão trocou para `sonnet` por instrução explícita do usuário antes desta fase — registrado, não repetido. Próximo: Fase 2 (T4–T6, `UBattleTurnCoordinator` — T6 é 🧠, corrida entre os dois commits). Design resolve os 4 decision points: DP-online-01 autoridade no código / topologia no deploy (com ressalva registrada de que `ListenServer` nunca é competitivo, porque o host lê o commit do oponente e isso quebra BTL-02); DP-online-02 RPC nativo (não serviço HTTP — contas de jogador não existem até M7); DP-online-03 reconexão por `FBattleState` completo, sem replay de trace; DP-online-04 timeouts como constantes nomeadas. **Decisão técnica derivada de investigação, não de preferência:** o commit não viaja como `FTurnCommit` (que tem array C estático `Actions[3]`), e sim como `FNetTurnCommit` de campos nomeados — verifiquei que o RPC *compila*, mas não consegui provar que `FRepLayout` expande o array interno em vez de serializar só `Actions[0]`, e esse modo de falha é silencioso. Ver L-017 (descoberta colateral dessa investigação). Fases 1–6 da Apresentação do Combate concluídas antes disto.
