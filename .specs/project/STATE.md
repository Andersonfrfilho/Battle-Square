...
**Trabalho atual:** **Legibilidade do Combate + Oponente com Critério.** O combate estava jogável e ilegível: o núcleo distingue seis desfechos e nenhum chegava ao jogador. Agora cada pet mostra barra de vida (o `HealthRatio` já era calculado e jogado fora uma linha antes de virar tela), cada evento vira frase em português no rodapé, e o feed é PRODUTO — compila em Shipping e não some com `bs.ShowBattleDebug 0`. **Localização criada do zero**, porque não existia nada: alvo `Game` com pt-BR nativa mais en e es, narração em `FText`/`LOCTEXT` com argumentos NOMEADOS (`{Actor}`, não `{0}` — em alemão o objeto vem antes do verbo). Coleta verificada de verdade: 32 frases, `.locres` gerado para os três idiomas; faltava `ResourceName` no último passo, sem o qual o `.locres` nunca nasce. **Oponente com critério** substituiu o sorteio uniforme, que anulava a legibilidade recém-entregue — contra uma moeda não há padrão para ler. Ele aproxima, ataca ao alcance, se protege ferido, e SIMULA a própria posição nos três slots (a IA antiga decidia os três da casa inicial, então "aproximar e atacar" era impossível para ela). `FDumbOpponentAI` foi preservado de propósito: é instrumento de medição do balanceamento, onde jogo aleatório isola efetividade de tipo da tática. **Pets se olham**, com marca que orbita — girar uma esfera simétrica seria invisível. Bateria: **202/202** (149 BattleSquare + 53 BattleSim), quatro sondas limpas (a nova `audit_localizable_text.sh` incluída). ABERTO: DP-ia-04 (camuflagem, voo, subsolo são MECÂNICAS, não visual — proposta escrita, não implementada).
